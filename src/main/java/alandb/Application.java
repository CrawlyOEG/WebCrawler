...
public class Application {
	...
	public void run() {
		...
		waitJavaScript();
		if(driver.getPageSource().contains("Sorry, but your query didn't produce any results!")) {
			System.out.println("No results found in Darsky.");
			return;
		}
	...
	public void extract(HashMap<String,String> uris){
		...
		uris.forEach((k, v) -> {
			if(comprobarRecorrido(k)) {
				...
				System.out.println("Downloading: "  + driver.findElement(By.cssSelector("td[class='mainfieldsbg'][colspan='3']")).getText());
				try {
